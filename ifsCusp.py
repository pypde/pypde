...
class InterfaceSolverCusp(object):
    ...
    #
    typeName = 'cusp'
    def __init__(self,msh,refState,vis1=1.,cLim=1e-5,dLim =.5):
        self.__vis1 = vis1
        ...
        #self.__msh = msh
    #---------------------------------------------------------------------------
    def interfaceNormalFlux (self,wL,wR,fL,fR,
                             sF,msh,
                             w=None,mode='interior'):
        ...
        if mode == 'boundary':
            ...
            ## use SLIP to CORRECT FACE VALUES (FROM LINEAR RECONSTRUCTION)
            ...
            qsL = sF[0,:]*uL + sF[1,:]*vL
            qsR = sF[0,:]*uR + sF[1,:]*vR
            qsF = .5*(qsL + qsR) #qsF = sF[0,:]*uF + sF[1,:]*vF
            csF = cF*normSF
            qmF = qsF/csF
            #print cF.shape
            #print normSF.shape
